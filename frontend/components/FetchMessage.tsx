...
const FetchMessage = () => {
  ...
  const hasFetchedRef = useRef(false); // To prevent double calls
  ...
  const handleOpenRouter = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        ...
        // Append the new chunk to existing response
  ...
  }, [bearerToken, currentModel, conversationId, currentMessage]);

  useEffect(() => {
    if (fetchMessagesinchunk && !hasFetchedRef.current) {
      handleOpenRouter();
    }
  }, [fetchMessagesinchunk, handleOpenRouter]);

  // Cleanup function when component unmounts or when streaming completes
  useEffect(() => {
    return () => {
  ...
  // Reset states when streaming completes
  useEffect(() => {
    if (!isStreaming && streamingResponse && hasFetchedRef.current) {
      // Small delay to ensure message is displayed before clearing
      const timer = setTimeout(() => {
        dispatch(setCurrentMessage(''));
        dispatch(setFetchMessagesInChunk(false));
        hasFetchedRef.current = false; // Reset for next time
      }, 100);
      
      return () => clearTimeout(timer);
    }
  }, [isStreaming, streamingResponse, dispatch]);
